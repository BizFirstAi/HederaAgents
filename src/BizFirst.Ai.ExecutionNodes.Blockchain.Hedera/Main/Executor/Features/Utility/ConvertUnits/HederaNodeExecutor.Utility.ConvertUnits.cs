...
{
    private async Task<NodeExecutionResult> _Hedera_Utility_ConvertUnits_Async(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            //code-step: 1.3 - Call Hedera utility service to convert units (local, no network)
            var r = _utilityService.ConvertUnits(info.Amount!, info.FromUnit, info.ToUnit);
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/convertUnits failed for '{info.Amount}': {ex.Message}", this);
        }
    }
}
