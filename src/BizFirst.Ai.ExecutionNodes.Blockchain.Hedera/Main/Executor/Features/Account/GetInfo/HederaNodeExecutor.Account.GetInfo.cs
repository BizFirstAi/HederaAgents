...
{
    private async Task<NodeExecutionResult> _Hedera_Account_GetInfo_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to AccountGetInfoInfo
        if (mySettings?.ActiveInfo is not AccountGetInfoInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Hedera account service to fetch account info (Mirror Node REST)
            var r = await _accountService.GetInfoAsync(info.Network, info.AccountId!, cancellationToken);
            ...
            //code-step: 1.5 - Extract account info record from result
            var record = new Dictionary<string, object>
            {
                { "accountId", r.AccountId },
                { "evmAddress", r.EvmAddress ?? string.Empty },
                { "memo", r.Memo ?? string.Empty },
                { "autoRenewPeriodSeconds", r.AutoRenewPeriodSeconds ?? 0 },
                { "deleted", r.Deleted },
            ...
            //code-step: 1.7 - Convert account info record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"account/getInfo failed for {info.AccountId}: {ex.Message}", this);
        }
    }
}
