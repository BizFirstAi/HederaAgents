...
## Roadmap: signed writes

Signed writes (account create/transfer, token mint/associate/transfer, topic create/submit, message sign, etc.)
follow the same partial/service pattern as the read operations above. They require the Hedera .NET SDK — the
community **`Hashgraph`** package (bugbytesinc) is the intended dependency, since the `Hiero` successor
currently targets net10 only. The signing credential is resolved from a vault `CRYPTO_WALLET` record, plus the
payer account id and key algorithm supplied as config fields.
